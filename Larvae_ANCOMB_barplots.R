...
egg=ggplot(subset(comp_l, Diff_more_abundant =="Egg"), aes(x=reorder(label,abs_W),y=abs_W, label = label)) + 
  geom_text(aes(y = abs_W), vjust=0, size =2, hjust=0) + geom_bar(stat = "identity", 
...
pdf("outputs/Larvae_biomarkers.pdf", width = 6, height = 4, pointsize = 6)
egg+spe+sea
dev.off()

pdf("outputs/Larvae_biomarkers_vert.pdf", width = 3, height = 8, pointsize = 6)
egg/spe/sea
dev.off()
